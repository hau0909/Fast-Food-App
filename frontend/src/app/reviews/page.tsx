...
export default function AdminReviewsPage() {
	...
	function ReviewStar({ filled, className }: { filled?: boolean; className?: string }) {
	...
	const [selectedForDelete, setSelectedForDelete] = useState<Review | null>(null)
	...
	const fetchReviews = async () => {
		...
		try {
			...
			const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/reviews${q}`, {
					...
					const list = Array.isArray(data?.data) ? data.data : Array.isArray(data) ? data : []
							setReviews(
								list.map((r: any) => ({
									...
									username: r.user_id?.username ?? r.user?.username ?? r.author ?? undefined,
	...
	const updateApprove = async (id: string, approve = true) => {
		...
		try {
			const token = getToken()
			const url = `${process.env.NEXT_PUBLIC_API_URL}/api/admin/reviews/${id}/${approve ? "approve" : "hide"}`
			...
			const updatedApproved = updated.approved ?? updated.is_approved ?? updated.isApproved ?? false
			setReviews((prev) => prev.map((r) => (r._id === updated._id ? { ...r, approved: updatedApproved } : r)))
	...
	const deleteReview = async (id?: string) => {
		...
		try {
			const token = getToken()
			await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/reviews/${id}`, {
				withCredentials: true,
				headers: token ? { Authorization: `Bearer ${token}` } : undefined,
			})
				...
				<main className="col-span-12 md:col-span-9 lg:col-span-10 p-8">
					<div className="mx-auto max-w-7xl">
						<div className="flex justify-between items-center mb-8">
							<div>
								<div className="flex items-center gap-3 mb-2">
									...
									<h1 className="font-bold text-foreground text-3xl">Đánh giá</h1>
								</div>
								<p className="text-muted-foreground">Quản lý đánh giá của khách hàng</p>
							</div>
							<div className="flex items-center gap-3">
								<div className="flex items-center gap-2">
									<label className="text-muted-foreground text-sm">Filter</label>
									<select value={filterApproved === null ? "all" : filterApproved} onChange={(e) => setFilterApproved(e.target.value === "all" ? null : e.target.value)} className="bg-background px-3 py-2 border border-input rounded-md text-foreground text-sm">
						...
						{loading ? (
							...
							<div className="bg-destructive/10 p-4 border border-destructive/20 rounded-lg text-destructive">{error}</div>
						) : reviews.length === 0 ? (
							...
							<div className="gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
								{reviews.map((r) => (
									<div key={r._id} className="flex flex-col bg-card hover:shadow-md p-4 border border-border rounded-lg overflow-hidden transition-shadow">
										<div className="flex items-start gap-3">
																							<div className="flex justify-center items-center bg-muted rounded-full w-12 h-12 font-semibold text-foreground text-sm">
																								{((r.username ?? r.userEmail ?? "K").charAt(0) || "K").toUpperCase()}
																							</div>
											<div className="flex-1">
												<h3 className="mb-1 font-semibold text-foreground">{r.username ?? r.userEmail ?? "Khách"}</h3>
																		{r.productName ? <p className="text-muted-foreground text-sm">Sản phẩm: {r.productName}</p> : null}
																			...
																			<div className="flex items-center gap-0.5" aria-hidden>
																											{Array.from({ length: 5 }).map((_, i) => (
																												<ReviewStar
																													key={i}
																													filled={i < (r.rating ?? 0)}
																													className={`w-4 h-4 ${i < (r.rating ?? 0) ? "text-amber-400" : "text-muted-foreground/50"}`}
																												/>
																											))}
																			</div>
																			<span className="text-muted-foreground text-sm">{r.rating ?? 0}/5</span>
																		</div>
																		<p className="mt-2 text-muted-foreground text-sm truncate">{r.text}</p>
												<p className="mt-2 text-muted-foreground text-xs">Tạo: {r.createdAt ? new Date(r.createdAt).toLocaleString() : ""}{r.updatedAt ? ` • Cập nhật: ${new Date(r.updatedAt).toLocaleString()}` : ""}</p>
											</div>
										</div>

										<div className="flex justify-between items-center gap-2 mt-4">
											<span className={`inline-block px-2 py-1 rounded text-xs font-medium ${r.approved ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-700"}`}>
												{r.approved ? "Approved" : "Hidden"}
											</span>
											<div className="flex gap-2">
												{r.approved ? (
													<Button variant="outline" size="sm" onClick={() => updateApprove(r._id, false)} className="gap-1">
														<EyeOff className="w-4 h-4" /> Hide
													</Button>
												) : (
													<Button variant="outline" size="sm" onClick={() => updateApprove(r._id, true)} className="gap-1">
														<Check className="w-4 h-4" /> Approve
													</Button>
												)}

												<Dialog open={!!selectedForDelete && selectedForDelete._id === r._id} onOpenChange={(open) => !open && setSelectedForDelete(null)}>
													<DialogTrigger asChild>
														<Button variant="destructive" size="sm" onClick={() => setSelectedForDelete(r)} className="gap-1">
													...
													<DialogContent className="max-w-md">
														...
														<p className="text-muted-foreground text-sm">Bạn có chắc muốn xóa đánh giá này? Hành động này không thể hoàn tác.</p>
														<div className="flex gap-2 pt-4">
															<Button variant="outline" onClick={() => setSelectedForDelete(null)} className="flex-1">Hủy</Button>
															<Button variant="destructive" onClick={() => deleteReview(r._id)} className="flex-1" disabled={isSubmitting}>{isSubmitting ? "Đang xóa..." : "Xóa"}</Button>
														</div>
													</DialogContent>
												</Dialog>
											</div>
